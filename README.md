...
- Project status: **mid implementation** — config (parse/validate/three-source resolution), the markdown span locator, the frontmatter read projection, `Note` loading, the features-directory scan, the `list` command, the git layer, and the create path (`new`: branch→id inference, template rendering with the four-layer value map, `repo`/`description` fills, refuse-to-overwrite write) are landed and tested — 124 tests across unit and integration tiers. Git specifics: a `gix` adapter (branch/origin discovery) behind a crate-private wall with `context()` as its only fact, and the `Context` object carries the git snapshot (branch + repo identity) derived tolerantly from the working directory — every command can ask "which ticket am I on, which repo is this" without touching git internals. `open` is accepted-but-unimplemented. See `PLAN.md` for design decisions, remaining work, and open questions.
