...
- [ ] **Phase 6 — write path:** D11 machinery (field-region edits, multi-edit one-pass rebuild, atomic temp+rename); `status`, the append commands (D19), `fm` (D16). (`ftypes` — D31 — and the `Status` vocabulary — D37 — already extracted.)
